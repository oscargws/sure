...
module RedbarkItem::Unlinking
  ...
  def unlink_all!(dry_run: false)
    ...
    redbark_accounts.find_each do |lfa|
      links = AccountProvider.where(provider_type: "RedbarkAccount", provider_id: lfa.id).to_a
      ...
      begin
        ...
        Rails.logger.warn(
          "RedbarkItem Unlinker: failed to fully unlink LFA ##{lfa.id} (links=#{link_ids.inspect}): #{e.class} - #{e.message}"
        )
